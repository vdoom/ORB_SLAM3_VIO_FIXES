/**
 * @file mono_inertial_realsense_D455_VIO_v2.cc
 * @brief Monocular-Inertial VIO implementation with coordinate frame bridging for ArduPilot integration
 *
 * Based on stereo_inertial_realsense_D455_VIO_v2.cc but using a single IR camera
 * from the RealSense D455 in monocular-inertial mode.
 *
 ...
 * - --test mode for running without MAVLink (with Pangolin visualization)
 *
 * Key difference from stereo version: Uses IMU_MONOCULAR mode with TrackMonocular()
 * and only the first IR camera stream. Scale comes from IMU integration rather than
 * stereo baseline, so initialization requires camera rotation.
 *
...
public:
    VIOBridge(MAVLinkMode mode = MAVLinkMode::VISION_POSITION_ESTIMATE) {
        ...
        std::cout << "==================================================" << std::endl;
        std::cout << "[VIOBridge v2] Mono-Inertial with coordinate bridging" << std::endl;
        std::cout << "==================================================" << std::endl;
...
int main(int argc, char **argv) {
    std::cout << "==================================================" << std::endl;
    std::cout << "ORB-SLAM3 Mono-Inertial VIO v2 (ArduPilot Bridge)" << std::endl;
    std::cout << "  Using RealSense D455 left IR camera + IMU" << std::endl;
    std::cout << "==================================================" << std::endl;
    ...
    if(argc < 3 || (argc > 5 && !test_mode) || (argc > 4 && test_mode)) {
        ...
        std::cerr << "  Normal:  ./mono_inertial_realsense_D455_VIO_v2 path_to_vocabulary path_to_settings [mode] [visualization]" << std::endl;
        std::cerr << "  Test:    ./mono_inertial_realsense_D455_VIO_v2 path_to_vocabulary path_to_settings --test" << std::endl;
        std::cerr << std::endl;
        ...
        std::cerr << std::endl;
        std::cerr << "NOTE: Mono-inertial requires camera ROTATION during initialization" << std::endl;
        std::cerr << "      for IMU-based scale estimation." << std::endl;
        return 1;
    }
    ...
    // Create SLAM system - MONOCULAR-INERTIAL mode
    std::cout << "[Main] Loading ORB-SLAM3 (Mono-Inertial)..." << std::endl;
    ORB_SLAM3::System SLAM(argv[1], argv[2], ORB_SLAM3::System::IMU_MONOCULAR, enable_visualization);
    ...
    // Configure RealSense - single IR camera + IMU
    std::cout << "[Main] Configuring RealSense D455 (mono IR + IMU)..." << std::endl;
    rs2::pipeline pipe;
    rs2::config cfg;

    // Only left IR camera (stream index 1)
    ...
    // IMU callback - single IR frame + IMU
    auto imu_callback = [&](const rs2::frame& frame)
    {
        ...
        if(rs2::frameset fs = frame.as<rs2::frameset>())
        {
            ...
            // Get single IR frame (left camera)
            rs2::video_frame ir_frame = fs.get_infrared_frame();
            imCV = cv::Mat(cv::Size(width_img, height_img), CV_8U, (void*)(ir_frame.get_data()), cv::Mat::AUTO_STEP);
    ...
    std::cout << "[Main] Starting Mono-Inertial VIO tracking..." << std::endl;
    std::cout << "[Main] IMPORTANT: Move camera with ROTATION to initialize (mono-inertial needs rotation for scale)." << std::endl;
