/**
 * @file stereo_inertial_pear_direct.cc
 * @brief Stereo-Inertial VIO with PearAPI Direct Stereo + MAVLink/ArduPilot Integration
 *
 * Same as stereo_inertial_pear.cc but uses Direct Stereo mode instead of
 * shared memory (Shm). Both cameras run in-process via CameraBackend::create()
 * with rawCapture=true, bypassing the ISP to avoid the dual-camera FPS penalty.
 *
 * RPi5 has two independent ISP front-ends, but running two libcamera cameras
 * through the ISP in one process still bottlenecks at ~13 FPS. Raw capture mode
...
// PearAPI for camera, IMU, and stereo frame pairing
#include <PearAPI/PearAPI.h>
#include <PearAPI/CameraBackend.h>
...
int main(int argc, char** argv) {
    ...
    cout << "=================================================" << endl;
    cout << "Stereo-Inertial VIO (PearAPI Direct Stereo + MAVLink)" << endl;
    cout << "=================================================" << endl;
    if (test_mode) {
        cout << "  *** TEST MODE - No MAVLink, Visualization ON ***" << endl;
    }
    cout << "Camera mode:     DIRECT STEREO (in-process, rawCapture=true)" << endl;
    ...
    // ---- Initialize stereo cameras using Direct Stereo mode ----
    // Both cameras run in-process with rawCapture=true to bypass the ISP,
    // avoiding the dual-camera FPS penalty (~13 FPS -> ~26 FPS per camera).
    pearvio::CameraConfig camConfig;
    camConfig.cameraIndex = 0;
    camConfig.fps = 240;  // Request max FPS (sensor/trigger will limit actual rate)
    camConfig.rawCapture = true;  // Direct stereo: bypass ISP for raw capture
    ...
    // Force rawCapture for direct stereo (overrides config file)
    camConfig.rawCapture = true;
    ...
    // avoiding the dual-camera FPS penalty (~13 FPS -> ~26 FPS per camera).
    //
    // IMPORTANT: Initialization order matters for libcamera. Must fully
    // initialize + start cam0 before initializing cam1, matching PearCameraApp's
    // direct stereo sequence. Otherwise cam0 may not produce frames.
    cout << "Initializing stereo cameras in Direct Stereo mode (rawCapture=true)..." << endl;

    // ---- Set up stereo frame pairing (before starting cameras) ----
    struct StereoFrame {
        cv::Mat left;
        cv::Mat right;
        uint64_t timestamp;
    ...
    // ---- cam0: initialize, start, apply settings (NO callback yet) ----
    pearvio::CameraConfig camConfig0 = camConfig;
    camConfig0.cameraIndex = 0;
    auto cam0 = pearvio::CameraBackend::create();
    if (!cam0) {
        cerr << "Failed to create camera backend for cam0" << endl;
        return 1;
    }
    if (!cam0->initialize(camConfig0)) {
        cerr << "Failed to initialize cam0 (direct mode)" << endl;
        return 1;
    }
    cout << "cam0 initialized: " << cam0->frameWidth() << "x" << cam0->frameHeight()
         << " (raw=" << camConfig0.rawCapture << ")" << endl;
    ...
    // Apply cam0 settings immediately after start
    ...
    auto cam1 = pearvio::CameraBackend::create();
    if (!cam1) {
        cerr << "Failed to create camera backend for cam1" << endl;
        cam0->stop();
        return 1;
    }
    if (!cam1->initialize(camConfig1)) {
        cerr << "Failed to initialize cam1 (direct mode)" << endl;
        cerr << "Check that a second CSI camera is connected." << endl;
        cam0->stop();
        return 1;
    }
    cout << "cam1 initialized: " << cam1->frameWidth() << "x" << cam1->frameHeight()
         << " (raw=" << camConfig1.rawCapture << ")" << endl;
    ...
    // Apply cam1 settings immediately after start
    ...
    // ---- NOW connect frame callbacks after both cameras are configured ----
    // Wait for trigger mode to settle and flush free-running frames
    ...
    cout << "Frame callbacks connected (after trigger mode settled)" << endl;

    cout << "Both cameras started in direct mode" << endl;
    ...
    while (!SLAM.isShutDown() && b_continue_session) {
        ...
        // Validate frames before passing to ORB-SLAM3
    ...
    // No ShmCameraLauncher cleanup needed -- direct mode has no server processes
    ...
    cout << "=================================================" << endl;
    cout << "Session complete (Direct Stereo mode)" << endl;
